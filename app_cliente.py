...
# --- CONFIGURAÇÃO E URL ---
# Captura o parâmetro '?prestador=...' da URL
params = st.query_params
prestador_alvo = params.get("prestador")

# Validação do prestador na URL
if not prestador_alvo:
    st.error("Erro: Link inválido. Por favor, utilize o QR Code do seu prestador.")
    st.stop()

# URLs do Firebase (Dinamizadas pelo prestador)
...
st.markdown("""
    ...
    .big-box { background-color: #1a1a1a; padding: 20px; border-radius: 10px; border: 1px solid #333; margin-bottom: 20px; }
    </style>
...
if not st.session_state.registado:
    ...
    if busca:
        try:
            ...
            st.error("Erro ao conectar com o catálogo. Tente novamente.")
    ...
    if st.button("Enviar Pedido Manual"):
        if pedido_manual:
            ...
            st.warning("Digite o nome da música primeiro.")
    st.markdown('</div>', unsafe_allow_html=True)
